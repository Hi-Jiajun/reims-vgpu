...
pub fn gfx_write<H: HostMemory + HostOps>(
    ...
    match offset {
        ...
        GFX_REG_VERSION => {
            ...
            // and its display pipe does not exist yet, so the ~2 s the import
            // costs on a multi-gigabyte RAMBlock is spent where nothing is
            // timing it. Left lazy it lands inside the first draw, inside a
            // display transaction the guest abandons after 1000 ms — see
            // `guest_ram_map::resolve` for the measurement and
            // `guest_ram_map::warm` for why it must not run before the backend
            // has published a granularity.
