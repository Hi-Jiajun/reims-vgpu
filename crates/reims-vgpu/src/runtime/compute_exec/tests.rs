...
fn a_tiled_launch_is_byte_identical_across_the_engine_and_the_canonical_provider() {
    ...
    for (grid, regions) in [([4u32, 1, 1], 1usize), ([6, 1, 1], 2), ([10, 1, 1], 2)] {
        ...
        // The engine's object caches live in a caller-owned `DeviceState` now, so
        // this arm keeps one per process, as upstream's engine suites do.
        static ENGINE_STATE: std::sync::OnceLock<crate::model::DeviceState> =
            std::sync::OnceLock::new();
        let engine_state = ENGINE_STATE.get_or_init(|| {
        ...
        let engine =
            vk_engine::execute_compute_request(engine_state, &request).expect("the engine runs");
...
fn run_mul3add1_dispatch(
    ...
    let mut bindings = vec![buf_bind(buffer_ref, 0)];
