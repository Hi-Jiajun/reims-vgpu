...
/// A multi-row type-11 copy vouches the destination mapping's pages **once**,
/// not once per row.
///
/// The vouch (`mapper::type4_pages_witness`, behind `vouch_for_write`) re-walks
/// the mapping's whole guest page table. Its cost is therefore O(pages in the
/// surface) per call, and the staging loop used to make one call per row — so a
/// copy cost O(rows x pages). On a real surface that is not a constant factor: a
/// 1920x1080 window is 2025 pages and 1080 rows, and a driven macos-13 Maps boot
/// measured 22 blit records costing 19.4 s between them.
///
/// Asserted on the counter rather than on a timing, because the timing is the
/// symptom and the call count is the property. Sixteen rows against one vouch is
/// the same statement a thousand rows would make and does not need a big
/// fixture; before the change this read 16.
#[test]
fn a_multi_row_type11_copy_vouches_the_destination_once() {
    use crate::runtime::drain::store_route_count;
    let (mut host, mut state) = blit_device();
    const ROWS: u32 = 16;
    install_type11_sized(&mut host, &mut state, 3, 3, 0x20, ROWS);
    install_type11_sized(&mut host, &mut state, 4, 4, 0x21, ROWS);

    // A distinct byte per texel, so a row landing at the wrong y is visible.
    let src: Vec<u8> = (0..(2 * ROWS * 4)).map(|i| (i % 251) as u8).collect();
    assert!(mapping_write::write_rect_raw(
        &mut state,
        &mut host,
        3,
    ...
    // Every verdict the guard can reach, summed: the fixture's mapping carries
    // no type-4 walk, so it answers `Unwitnessed` rather than `Ours`, and the
    // property under test is how many times the guard *ran* — which is one walk
    // of the page list either way.
    let guard_calls = || {
        store_route_count("mapw_pages_vouched")
            + store_route_count("mapw_pages_unwitnessed")
            + store_route_count("mapw_pages_refused")
    };
    let before = guard_calls();
    let mut cmd = copy_cmd(CopyKind::TextureToTexture, 3, 4);
    cmd.source_size = Size {
        width: 2,
        height: ROWS as u64,
    ...
    assert_eq!(execute_blit(&mut state, &mut host, 1, &cmd), BlitStatus::Ok);
    let vouches = guard_calls() - before;

    // The content first: a call-count assertion on a copy that lost pixels
    ...
    assert_eq!(back, src, "every row of the copy landed at its own y");
    assert_eq!(
        vouches, 1,
        "one page-table walk for the whole copy, not one per row"
    );
}

/// [`install_type11`] with a caller-chosen height, for the multi-row copies.
fn install_type11_sized(
    host: &mut FakeHost,
    state: &mut DeviceState,
    obj_ref: u32,
    mapping_id: u32,
    ...
    assert!(state.set_mapping_geom(mapping_id, 2, height, MTL_FORMAT_BGRA8_UNORM));
